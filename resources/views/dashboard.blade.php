    ...
    <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8 relative">
      ...
      <a href="{{ route('product.show', $product->id) }}" class="group">
        <div class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-lg bg-gray-200 xl:aspect-h-8 xl:aspect-w-7">
          <img src="{{ asset('storage/'. $product->productIcon) }}" alt="Tall slender porcelain bottle with natural clay textured body and cork stopper." class="h-full w-full object-cover object-center group-hover:opacity-75">
        </div>
          @php $harga = number_format($product->productPrice, 0, ',', '.'); @endphp
          <h3 class="mt-4 text-sm text-gray-700">{{ $product->productName }}</h3>
          <p class="mt-1 text-lg font-medium text-gray-900">Rp{{ $harga }}</p>
        <div class="-mt-10">
